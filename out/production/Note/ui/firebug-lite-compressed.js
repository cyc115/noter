var firebug = {
    ...
    init: function (_css) {
        ...
        with(firebug) {
            ...
            el.content = {};
            el.mainiframe = new lib.element("IFRAME").attribute.set("id", "FirebugIFrame").attribute.set('firebugIgnore', true).environment.addStyle({
                "display": "none",
                "width": lib.util.GetViewport().width + "px"
            }).insert(document.body);
            el.main = new lib.element("DIV").attribute.set("id", "Firebug").attribute.set('firebugIgnore', true).environment.addStyle({
                "display": "none",
                "width": lib.util.GetViewport().width + "px"
            }).insert(document.body);
            if (!internal.isPopup) {
            ...
            if (lib.env.ie || lib.env.webkit) {
                el.button.container.environment.addStyle({
                    "paddingTop": "12px"
                });
            }
            el.nav = {};
    ...
    win: {
        ...
        minimize: function () {
            with(firebug) {
                ...
                el.button.maximize.environment.addStyle({
                    "display": "block"
                });
                el.button.minimize.environment.addStyle({
                    "display": "none"
                });
                win.refreshSize();
            }
        ...
        newWindow: function () {
            var interval, scripts, script, scriptPath, fe = firebug.env,
                fi = firebug.internal;
            if (!fi.popupWin) {
                ...
                if (!fi.popupWin) {
                    ...
                    if (scriptPath) {
                            ...
                            if (!done && (!firebug.lib.env.ie || this.readyState == "complete" || this.readyState == "loaded")) {
                                ...
                                if (fi.popupWin.firebug) {
                                    with(fi.popupWin.firebug) {
                                        ...
                                        env.css = fe.css;
                                        init();
                                        el.button.dock.environment.addStyle({
                                            "display": "block"
                                        });
                        ...
                        if (!done) {
                            ...
                            firebug.el.mainiframe.environment.addStyle({
                                "display": "none"
                            });
                        }
                    } else {
        ...
        resizer: {
            ...
            resize: function (_event) {
                with(firebug) {
                    ...
                    if (lib.env.ie6) {
                        el.main.environment.addStyle({
                            "top": win.resizer.y[3] - (win.resizer.y[1] - _event.clientY) + "px"
                        });
                        el.mainiframe.environment.addStyle({
                            "top": win.resizer.y[3] - (win.resizer.y[1] - _event.clientY) + "px"
                        });
        ...
        setVerticalPosition: function (_dim, _event) {
            with(firebug) {
                ...
                el.main.environment.addStyle({
                    "top": dim.height - el.main.environment.getSize().offsetHeight + Math.max(document.documentElement.scrollTop, document.body.scrollTop) + "px"
                });
                el.mainiframe.environment.addStyle({
                    "top": dim.height - el.main.environment.getSize().offsetHeight + Math.max(document.documentElement.scrollTop, document.body.scrollTop) + "px"
                });
    ...
    d: {
            ...
            print: function (_object, _parent, _inTree) {
                with(firebug) {
                    ...
                    if (parentElement.opened && parentElement != el.left.dom.container) {
                        ...
                        parentElement.environment.addStyle({
                            "display": "none"
                        });
                        return;
                    }
                    ...
                    for (var key in obj) {
                        try {
                            ...
                            var value = obj[key],
                                property = key,
                                container = new lib.element("DIV").attribute.addClass("DOMRow").insert(parentElement),
                                left = new lib.element("DIV").attribute.addClass("DOMRowLeft").insert(container),
                                right = new lib.element("DIV").attribute.addClass("DOMRowRight").insert(container);
        ...
        html: {
            ...
            highlight: function (_element, _clear, _event) {
                with(firebug) {
                    ...
                    if (_clear) {
                        internal.targetWindow.firebug.el.bgInspector.environment.addStyle({
                            "display": "none"
                        });
                        return;
                    }
            ...
            inspect: function (_element) {
                var map = [],
                    parentLayer, t, link, tagName, searchEl, parent = _element;
                while (parent) {
                    map.push(parent);
            ...
            toggle: function (_absoluteValue, _event) {
                with(firebug) {
                    ...
                    if (d.inspector.enabled == false) {
                        el.borderInspector.environment.addStyle({
                            "display": "none"
                        });
                        d.inspector.el = null;
        ...
        scripts: {
            ...
            open: function (_index) {
                with(firebug) {
                    ...
                    var i = 0,
                        script = document.getElementsByTagName("script")[_index],
        ...
        xhr: {
            ...
            open: function () {
                with(firebug) {
                    ...
                    el.left.xhr.name = new lib.element("DIV").attribute.addClass("BlockContent").insert(new lib.element("DIV").attribute.addClass("Block").environment.addStyle({
                        "width": "20%"
                    ...
                    el.left.xhr.status = new lib.element("DIV").attribute.addClass("BlockContent").insert(new lib.element("DIV").attribute.addClass("Block").environment.addStyle({
                        "width": "10%"
                    }).insert(el.left.xhr.container));
                    ...
                    el.left.xhr.readystate = new lib.element("DIV").attribute.addClass("BlockContent").insert(new lib.element("DIV").environment.addStyle({
                        "width": "15%"
                    }).attribute.addClass("Block").insert(el.left.xhr.container));
                    ...
                    el.left.xhr.response = new lib.element("DIV").attribute.addClass("BlockContent").insert(new lib.element("DIV").environment.addStyle({
                        "width": (lib.env.ie ? "50" : "55") + "%"
                    }).attribute.addClass("Block").insert(el.left.xhr.container));
        ...
        navigate: function (_index) {
            with(firebug) {
                ...
                env.dIndex = open;
                settings.hide();
                el.button[close].container.environment.addStyle({
                    "display": "none"
                });
                el.left[close].container.environment.addStyle({
                    "display": "none"
                });
                el.right[close].container.environment.addStyle({
                    "display": "none"
                });
                el.button[open].container.environment.addStyle({
                    "display": "inline"
                });
                el.left[open].container.environment.addStyle({
                    "display": "block"
                });
                el.right[open].container.environment.addStyle({
                    "display": "block"
                });
                if (el.nav[close])
